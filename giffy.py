...
from tkinter import ttk  # Import Progressbar from ttk

def gif_erstellen(datei_pfad, ziel_datei_pfad):
    """ Function to compress a video file to a gif
        datei_pfad: Pfad zur Videodatei
        ziel_datei_pfad: Pfad zur Zieldatei
    """
    ...
    subprocess.run(kommando, text=True, check=True)
    ...
    for ordnername, _, dateien in os.walk(quellpfad):
        ...
        for index, datei in enumerate(dateien, start=1):
            ...
            if os.path.exists(ziel_datei_pfad):
                print(f"{ziel_datei_pfad} existiert bereits. Überspringe das Komprimieren.")
            else:
...
TEXT_QUELLE = "Quellverzeichnis auswählen"  # Choose Source Directory in German
source_dir_button = tk.Button(app, text=TEXT_QUELLE, command=choose_source_directory)
...
TEXT_ERSTELLEN = "Gifs erstellen"  # Add compress button
